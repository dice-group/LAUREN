...
public class QaldEvaluatorApplication {
    ...
    public QaldEvaluatorApplication() {
    }
    ...
    private void evaluate(String components) throws IOException {
        ...
        for (QaldQuestion question : filereader.getQuestions()) {
            try{
                ...
                e.printStackTrace();
                logger.warn("Exception occured for {}", question.getQuestion());
            }
        }
        ...
        String filename = "./agdistis-salsa_results.txt";

        FileWriter fw = new FileWriter(filename, true);
        fw.write(components+'\n');
        fw.write("Global Precision" + String.valueOf(globalPrecision / count) + '\n');
        fw.write("Global Recall" + String.valueOf(globalRecall / count) + '\n');
        fw.write("Global F-measure" + String.valueOf(globalFMeasure / count) + '\n');
        fw.close();
    ...
    private ResultSet selectTripleStore(String namedGraph, String endpoint) {
        ...
        logger.info(String.valueOf(this.stadog5));
        if (this.stadog5) {
            endpoint += "/query";
        }
