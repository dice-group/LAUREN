...
from fetch_agdistis_entities import call_agdistis
...
dbpediaSPARQL="http://localhost:3030/ds/query" #"http://sparql.cs.upb.de:8891/sparql"
dbpediaSPARQL2="http://localhost:3030/ds/query" #"http://sparql.cs.upb.de:8891/sparql"
...
def evaluate(raw):
    ...
    for term in combinations:
        ...
        if not agdistis:
            ...
            if (word_is_verb(term, originalQuestion)) and (term[0].istitle() or len(term.split(' ')) < 2 or (
                    len(term) < 1 and len(searchIndex.ontologySearch(term)) > 2) or not (any(x.isupper() for x in term))):
    ...
    entities = call_agdistis(question)
    agdistis_entities = entities
...
if __name__ == '__main__':
    #datasets_evaluate()
    print(evaluate(['When was the Battle of Gettysburg?', 'PREFIX dbo: <http://dbpedia.org/ontology/>\nPREFIX res: <http://dbpedia.org/resource/>\nSELECT DISTINCT ?date \nWHERE { \n       res:Battle_of_Gettysburg dbo:date ?date .\n}', ['http://dbpedia.org/ontology/date'], ['http://dbpedia.org/resource/Battle_of_Gettysburg']]))
